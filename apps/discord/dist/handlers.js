...
// ── /fans gain ────────────────────────────────────────────
export async function handleFansGain(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: 'Trainer', value: stats.trainerName, inline: true }, { name: 'Club Tier', value: stats.clubRankTier, inline: true }, { name: 'Monthly Rank', value: stats.monthlyRank ? `#${stats.monthlyRank}` : 'N/A', inline: true }, { name: '\u200B', value: '\u200B', inline: false }, { name: 'Total Fans', value: formatFans(stats.totalFans), inline: true }, { name: `Gain (${periodLabel})`, value: `${gainSign}${formatFans(gain.value)}`, inline: true }, { name: 'Avg/Day', value: stats.avgDaily ? formatFans(stats.avgDaily) : 'N/A', inline: true }, { name: '\u200B', value: '\u200B', inline: false }, { name: 'Active Days', value: `${stats.activeDays}`, inline: true }, { name: '7d Gain', value: formatGain(stats.gain7d ?? stats.weeklyGain), inline: true }, { name: '30d Gain', value: stats.gain30d ? formatGain(stats.gain30d) : 'N/A', inline: true })
...
// ── /fans leaderboard ─────────────────────────────────────
export async function handleFansLeaderboard(interaction) {
    ...
    const lines = topN.map((s, i) => {
        ...
        switch (period) {
            case 'daily':
                gainDisplay = formatGain(s.dailyGain);
                break;
            case 'weekly':
                gainDisplay = formatGain(s.gain7d ?? s.weeklyGain);
                break;
            case 'monthly':
                gainDisplay = formatFans(s.monthlyFans);
                break;
        }
...
// ── /link add ─────────────────────────────────────────────
export async function handleLinkAdd(interaction) {
    if (!isAdmin(interaction)) {
        await interaction.reply({ content: '⛔ This command is admin-only.', ephemeral: true });
        return;
    }
...
// ── /link remove ──────────────────────────────────────────
export async function handleLinkRemove(interaction) {
    if (!isAdmin(interaction)) {
        await interaction.reply({ content: '⛔ This command is admin-only.', ephemeral: true });
        return;
    }
...
export async function handleLinkList(interaction) {
    ...
    const lines = links.map(l => `• <@${l.discordUserId}> → **${l.trainerName}** (${l.trainerId}) — linked <t:${Math.floor(new Date(l.linkedAt).getTime() / 1000)}:R>`);
    const embed = new EmbedBuilder()
...
export async function handleTrainerAutocomplete(interaction) {
    ...
    try {
        ...
        const filtered = members
            .filter(m => m.trainerName.toLowerCase().includes(query) ||
...
export async function routeCommand(interaction) {
    ...
    try {
        if (commandName === 'sync') {
            ...
            if (subcommand === 'gain')
                await handleFansGain(interaction);
            else if (subcommand === 'leaderboard')
                await handleFansLeaderboard(interaction);
            else
            ...
            if (subcommand === 'add')
                ...
                await handleLinkRemove(interaction);
            else if (subcommand === 'list')
                await handleLinkList(interaction);
            else
                await interaction.reply({ content: 'Unknown subcommand.', ephemeral: true });
        }
    }
