...
import { automationEngine, initScheduler } from './automation/index.js';
...
export async function startGatewayBot() {
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    // Direct bot mentions in guild channels
    if (client.user && message.mentions.has(client.user.id)) {
      await automationEngine.process(message);
      return;
    }
  });
  ...
  client.on(Events.ClientReady, async () => {
    ...
    // Initialize Phase 16 Automation Scheduler
  ...
  if (groqKey) {
    const primaryModel = process.env['AI_MODEL'] || process.env['GROQ_MODEL'] || 'openai/gpt-oss-120b';
    const fallbackModel = process.env['AI_FALLBACK_MODEL'] || 'openai/gpt-oss-20b';
    ...
    logger.warn('No GROQ_API_KEYS or GROQ_API_KEY set — services in local-brain / cache-only fallback mode.');
  ...
  const runScheduledTask = async (task: any) => {
    try {
      const aiService = createProvider((process.env['AI_PROVIDER'] as any) || 'groq', process.env['GROQ_API_KEYS'] || process.env['GROQ_API_KEY'] || process.env['OPENAI_API_KEY'] || '');
