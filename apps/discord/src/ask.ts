...
const ASK_TOOL_SLUGS = [
  ...
  searchKnowledgeTool.slug,
  retrieveDocumentTool.slug,
  summarizeDocumentTool.slug,
  listKnowledgeSourcesTool.slug,
];

/** Register /ask tools + skill tools + knowledge tools into the shared registry exactly once. */
export function ensureAskToolsRegistered(): void {
  ...
  registered = true;
  logger.info(`Registered ${askTools.length} /ask tools + ${allSkillTools.length} skill tools + ${allKnowledgeTools.length} knowledge tools`);
}
...
export async function handleAskAnswer(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    // Begin AI generation via shared generateAskResponse
    const question = record.question;
    const generatedAnswer = await generateAskResponse({
      userId,
      channelId,
      question,
      domainGuard: true,
      bypassTopicCheck: false,
    });
...
export interface GenerateAskResponseOptions {
  userId: string;
  channelId: string;
  question: string;
  domainGuard?: boolean;
...
/**
 * Shared core ask response generation used by both `/ask` and Direct Messages.
 * Unifies tools registration, safety check, optional taxonomy/domain filtering,
 * answer caching, conversation context retrieval, and ToolCallingAgent execution.
 */
export async function generateAskResponse(options: GenerateAskResponseOptions): Promise<string> {
  const { userId, channelId, question } = options;
  const domainGuard = options.domainGuard ?? false;
  const bypassTopicCheck = options.bypassTopicCheck ?? false;
  ...
  // Layer 3: taxonomy classification (only when topic check is not bypassed)
  if (!bypassTopicCheck) {
  ...
  try {
    await sharedUserMemoryStore.extractAndSaveMemory(userId, question);
  } catch (memErr: any) {
    logger.warn(`/ask memory extraction skipped: ${memErr?.message ?? memErr}`);
  }
  ...
  try {
    ...
    if (cached) {
      logger.info(`/ask cache hit for normalized question`);
  ...
  if (!generatedAnswer) {
    ...
    // Model off-topic check (only when domainGuard is active)
    if (domainGuard && isOffTopicAnswer(generatedAnswer)) {
  ...
  // Save exchange to persistent conversation memory
  await memoryService.saveUserMessage(userId, question, channelId);
  await memoryService.saveAssistantMessage(userId, generatedAnswer, channelId);
