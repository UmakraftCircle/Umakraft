...
import { linkRequestService } from './automation/link-request.js';
...
import { executeFanGain, getLeaderboard, getRank, executeMilestoneCheck, fanPaceService, automationEngine, clubLeaderDashboardService } from './automation/index.js';
...
export async function handleFansGain(interaction: ChatInputCommandInteraction) {
  ...
  if (!link) {
    await interaction.editReply(
      '⚠️ You are not linked to a trainer yet. Send `link` in DM to submit a link request to the Club Leader.'
    );
    return;
  }
...
export async function handleLinkRequestSlash(interaction: ChatInputCommandInteraction) {
  const reply = await linkRequestService.initiateLinkRequest(interaction.user.id, interaction.user.username);
...
export async function routeCommand(interaction: ChatInputCommandInteraction) {
  ...
  try {
    if (commandName === 'sync') {
      ...
      if (subcommand === 'request') await handleLinkRequestSlash(interaction);
      else if (subcommand === 'add') await handleLinkAdd(interaction);
      else if (subcommand === 'remove') await handleLinkRemove(interaction);
      else if (subcommand === 'list') await handleLinkList(interaction);
      else await handleLinkRequestSlash(interaction);
    ...
    } else if (commandName === 'fan-gain') {
      const res = await executeFanGain(interaction.user.id);
      await interaction.reply({ content: res });
    } else if (commandName === 'fan-leaderboard') {
      const circle = interaction.options.getString('circle', false) || 'unified';
      const period = interaction.options.getString('period', false) || 'daily';
      const res = await getLeaderboard(circle, period, 3);
      await interaction.reply({ content: res });
    } else if (commandName === 'rank') {
      const res = await getRank(interaction.user.id);
      await interaction.reply({ content: res });
    } else if (commandName === 'milestone') {
      const res = await executeMilestoneCheck(interaction.user.id);
      await interaction.reply({ content: res });
    } else if (commandName === 'pace') {
      const paceReport = await fanPaceService.getPaceForUser(interaction.user.id);
      const res = paceReport
        ? fanPaceService.formatPaceReportMessage(paceReport)
        : 'Fan tracking unavailable.';
      await interaction.reply({ content: res });
    } else if (commandName === 'club-health') {
      const res = await clubLeaderDashboardService.formatClubHealthReport();
      await interaction.reply({ content: res });
    } else if (commandName === 'risk-report') {
      const res = await clubLeaderDashboardService.formatRiskReport();
      await interaction.reply({ content: res });
    } else if (commandName === 'milestone-report') {
      const res = await clubLeaderDashboardService.formatMilestoneReport();
      await interaction.reply({ content: res });
    } else if (commandName === 'club-report') {
      const res = await clubLeaderDashboardService.formatWeeklyClubReport();
      await interaction.reply({ content: res });
    } else if (commandName === 'help') {
      const res = automationEngine.getHelpText();
      await interaction.reply({ content: res });
    }
  } catch (error: any) {
