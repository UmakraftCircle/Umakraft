 ...
 *
 * Reference module. Adapt the provider `complete` signature + message/tool
 * types to your real SDK (Groq/OpenAI), and wire into chat.ts / ask.ts so
    ...
    while (totalUsed < totalBudget) {
      ...
      for (const call of calls) {
        ...
        if (used >= budget) {
          ...
          return this.finalize(messages, "", finalizeReason, totalUsed, log);
        }
