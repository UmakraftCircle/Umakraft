...
import { GreetingService, DailyMessageService, MilestoneMessageService, MonthlyAchievementService, ReminderMessageService, RaceCommentaryService, promptLibrary, createProvider, loadRaceState, saveRaceState, buildRaceState } from '@ai-agent-platform/ai';
import type { TimeSlot, MilestoneInfo, MonthlyTier, TrainerGap, RacerData, RaceState } from '@ai-agent-platform/ai';
...
async function startGatewayBot() {
  ...
  if (groqKey) {
    ...
    logger.info('Services initialized: Greeting, Daily, Milestone, Monthly, Reminder, RaceCommentary');
  } else {
  ...
  // ── Daily Gap Reminder (linked trainers below 50M monthly) ──
  ...
  const sendGapReminder = async () => {
    try {
      ...
      for (const link of links) {
        ...
        if (monthly >= 50_000_000) continue; // graduated
        gaps.push({
      ...
      logger.info(
        `🎯 Gap reminder sent: ${gaps.length} trainer(s) below 50M monthly ` +
