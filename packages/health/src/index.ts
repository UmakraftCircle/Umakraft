...
/** Atomic JSON persistence, suitable for local development and container volumes. */
...
/** Writes one-line JSON logs that can be ingested by cloud logging drivers, Loki, or ELK. */
...
export function collectDeploymentHealth(env: NodeJS.ProcessEnv = process.env): CollectorResult[] {
  const isPandaStack = !!(env.PANDASTACK_ENVIRONMENT || env.PANDASTACK_SERVICE_ID || env.PANDA_ENV);
  const isRailway = !!(env.RAILWAY_ENVIRONMENT || env.RAILWAY_PROJECT_ID);
  const isGeneric = !!(env.DEPLOYMENT_ID || env.SERVICE_ID || env.ENVIRONMENT);

  if (!isPandaStack && !isRailway && !isGeneric) return [];

  const provider = isPandaStack ? 'PandaStack' : isRailway ? 'Railway' : 'Cloud';
  const environment = env.PANDASTACK_ENVIRONMENT || env.PANDA_ENV || env.RAILWAY_ENVIRONMENT || env.ENVIRONMENT || env.NODE_ENV;
  const serviceId = env.PANDASTACK_SERVICE_ID || env.RAILWAY_SERVICE_ID || env.SERVICE_ID;
  const deploymentId = env.PANDASTACK_DEPLOYMENT_ID || env.RAILWAY_DEPLOYMENT_ID || env.DEPLOYMENT_ID;
  const commit = env.PANDASTACK_GIT_COMMIT_SHA || env.RAILWAY_GIT_COMMIT_SHA || env.GIT_COMMIT_SHA || env.GITHUB_SHA;

  return [{
    service: 'deployment',
    level: 'info',
    message: `${provider} deployment environment detected`,
    metadata: {
      provider,
      environment,
      serviceId,
