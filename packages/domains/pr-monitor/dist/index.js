...
export class PRMonitorAPI {
    ...
    async fetchOpenPRs(repo) {
        ...
        try {
            ...
            const response = await fetch(`${this.baseUrl}/repos/${repo}/pulls?state=open&per_page=30`, { headers, signal: AbortSignal.timeout(10000) });
            if (!response.ok) {
    ...
    async fetchMyReviewRequests() {
        ...
        try {
            ...
            const response = await fetch(`${this.baseUrl}/search/issues?q=is:pr+review-requested:@me+type:pr+state:open&per_page=50`, {
                headers: {
...
export const prMonitorFetchPRs = {
...
export const prMonitorReviewRequests = {
