import { createLogger, CacheStore } from '@ai-agent-platform/shared';
...
export class FanTrackerAPI {
  ...
  private async apiGet<T>(path: string, cacheKey?: string, ttl?: number, forceRefresh = false): Promise<T> {
  ...
  /**
   * Invalidates internal cache for leaderboard/trainer stats or clears entire cache.
   */
  public invalidateCache(key?: string): void {
    if (key) {
      cache.delete(key);
      logger.info(`[FanTrackerAPI] Cache invalidated for key: ${key}`);
    } else {
      cache.clear();
      logger.info('[FanTrackerAPI] Cache invalidated: cleared all entries.');
    }
  }

  /**
   * Clears cache and immediately re-fetches fresh leaderboard data.
   */
  public async refreshLeaderboard(circle: CircleKey = 'unified'): Promise<TrainerStats[]> {
    this.invalidateCache();
    logger.info(`[FanTrackerAPI] Leaderboard refresh triggered for circle: ${circle}`);
    return this.fetchLeaderboard(circle, true);
  }
  ...
  public async fetchTrainerStats(trainerId: string, forceRefresh = false): Promise<TrainerStats> {
    ...
    try {
      ...
      return this.fetchTrainerStatsFromCircle(trainerId, forceRefresh);
    }
  }
   ...
   */
  private async fetchTrainerStatsFromCircle(trainerId: string, forceRefresh = false): Promise<TrainerStats> {
    const members = await this.fetchAllMembers(forceRefresh);
  ...
  public async fetchAllMembers(forceRefresh = false): Promise<TrainerStats[]> {
   ...
   */
  public async fetchLeaderboard(circle: CircleKey = 'unified', forceRefresh = false): Promise<TrainerStats[]> {
    if (circle === 'unified') return this.fetchAllMembers(forceRefresh);
