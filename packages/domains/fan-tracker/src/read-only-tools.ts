...
const logger = createLogger('FanTrackerReadOnlyTools');

/**
 * Feature 2: read-only, allow-listed tools over the existing fan-tracker data
 * source. These are the tools the /ask agent may invoke (never arbitrary
 * code/sql/network). All are side-effect free.
 */

export const getTrainerStatsTool: ToolDefinition = {
  slug: 'get_trainer_stats',
  name: 'Get Trainer Stats',
  description: 'Get fan count, ranks and tier for a single Uma Musume trainer by their numeric trainer id.',
  parameters: {
...
export const searchTrainersTool: ToolDefinition = {
...
export const getLeaderboardTool: ToolDefinition = {
  ...
  parameters: {
    ...
    top: { type: 'number', description: 'Number of top trainers to return (default 10)', required: false },
  },
  handler: async (args) => {
...
export const getUserProfileTool: ToolDefinition = {
  ...
  description: 'Resolve a Discord user id to their linked Uma Musume trainer profile (via trainerLinkStore + fanTrackerAPI).',
  parameters: {
...
export const readOnlyTrackerTools: ToolDefinition[] = [
  getTrainerStatsTool,
  searchTrainersTool,
  getLeaderboardTool,
  getUserProfileTool,
