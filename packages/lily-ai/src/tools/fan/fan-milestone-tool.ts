...
export class FanMilestoneTool implements LilyTool {
  public name = 'FanMilestoneTool';
  ...
  public canHandle(analysis: LanguageAnalysis): boolean {
    return analysis.intent === 'fan_milestone';
  }
  ...
  public async execute(context: ToolExecutionContext): Promise<ToolResult> {
    try {
      ...
      return {
        success: false,
        error: error.message || 'Failed to calculate fan milestones'
      };
    }
  }
}
