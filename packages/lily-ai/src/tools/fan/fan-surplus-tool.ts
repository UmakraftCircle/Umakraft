...
export class FanSurplusTool implements LilyTool {
  public name = 'FanSurplusTool';
  ...
  public canHandle(analysis: LanguageAnalysis): boolean {
    return analysis.intent === 'fan_surplus';
  }
  ...
  public async execute(context: ToolExecutionContext): Promise<ToolResult> {
    try {
      ...
      return {
        success: false,
        error: error.message || 'Failed to calculate fan surplus'
      };
    }
  }
}
