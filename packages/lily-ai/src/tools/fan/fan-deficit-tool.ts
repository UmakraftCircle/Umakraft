...
export class FanDeficitTool implements LilyTool {
  public name = 'FanDeficitTool';
  ...
  public canHandle(analysis: LanguageAnalysis): boolean {
    return analysis.intent === 'fan_deficit';
  }
  ...
  public async execute(context: ToolExecutionContext): Promise<ToolResult> {
    try {
      ...
      return {
        success: false,
        error: error.message || 'Failed to calculate fan deficit'
      };
    }
  }
}
