...
async function apiPost(url, headers, body, timeoutMs = DEFAULT_TIMEOUT_MS) {
...
export class OpenAIProvider {
    ...
    async #callWithRetry(messages, extra) {
        let lastError = new Error('No keys available for provider call');
        for (let attempt = 0; attempt < this.keys.length; attempt++) {
            const key = this.#pickKey(attempt > 0 ? this.keys[(attempt - 1) % this.keys.length] : undefined);
            const keySuffix = key.slice(-6);
            try {
                const result = await apiPost(`${this.baseUrl}/v1/chat/completions`, { Authorization: `Bearer ${key}` }, { model: this.model, messages, ...extra });
                ...
                if (isRateLimit && this.keys.length > 1 && attempt < this.keys.length - 1) {
                    logger.warn(`Key ...${keySuffix} rate-limited (429), rotating to next key ` +
                ...
                if (isTimeout && this.keys.length > 1 && attempt < this.keys.length - 1) {
                    logger.warn(`Key ...${keySuffix} timed out, rotating to next key ` +
                        `(attempt ${attempt + 2}/${this.keys.length})`);
                    continue;
                }
                throw err;
...
export function createProvider(type, apiKey, model) {
    switch (type) {
        ...
        case 'groq':
            return new OpenAIProvider(apiKey, model || 'llama-3.3-70b-versatile', 'https://api.groq.com/openai');
        default:
