...
export function CreateCompareSummaryService(): CompareSummaryService {
  ...
  if (groqKey) {
    ...
    try {
      fallback = createProvider('groq', groqKey, 'llama-3.3-70b-versatile');
    } catch {
      fallback = null;
    }
    ...
    logger.warn('No GROQ_API_KEYS or GROQ_API_KEY — compare summaries will use deterministic fallback.');
  }
