...
class SlotCache {
  ...
  get size(): number {
    return this.keys(`${this.slot}-`).length;
  }
...
export class DailyMessageService {
   ...
   * @param prompts     PromptLibrary with 'daily-message' template.
  ...
  /**
   * Generate a daily message for a specific time slot.
   *
   * 5-tier fallback, fully time-isolated:
   *   Morning failures → morning cache → morning bootstrap.
   *   Never crosses time slots (morning never recycles evening cache).
   */
  ...
  /** Clear a specific time slot's cache. */
  clearSlot(slot: TimeSlot): void {
    this.caches[slot].clear();
    logger.info(`Daily cache cleared: ${slot}`);
  }
  ...
  clearAll(): void {
    ...
    logger.info('All daily caches cleared.');
  }
  ...
  #fallbackDaily(timeSlot: TimeSlot): string {
    ...
    if (pool.length === 1) {
      ...
      if (msg) {
        logger.info(`Daily [${timeSlot}] fallback: sole cached message`);
        return msg;
      }
    }

    const bootstrapPool = BOOTSTRAP_POOLS[timeSlot];
    logger.warn(`Daily [${timeSlot}] fallback: cache empty — using random bootstrap (${bootstrapPool.length} ${poolLabel})`);
...
function bootstrapPoolLabel(slot: TimeSlot): string {
  switch (slot) {
    case 'morning':  return 'morning-themed messages';
    case 'noon':     return 'noon-themed messages';
    case 'evening':  return 'evening-themed messages';
    case 'midnight': return 'midnight-themed messages';
  }
}
