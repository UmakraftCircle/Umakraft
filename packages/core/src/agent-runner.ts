...
export class AgentRunner {
  ...
  async run(userId: string, goal: string, context?: { guildId?: string | null; channelId?: string | null }): Promise<AgentRunResult> {
    ...
    try {
      ...
      await persist({ status, completedStepsJson: JSON.stringify([...executed.tasks.keys()]), toolResultsJson: this.serializeResults(executed), errorsJson: JSON.stringify(errors), currentStep: executed.tasks.size });
  ...
  private async withOverallTimeout<T>(p: Promise<T>): Promise<T> {
  ...
  private synthesize(plan: ExecutionPlan, goal: string): string {
    ...
    const parts = completed.map((t, i) => {
      const r = t.result;
      const rendered = typeof r === 'string' ? r : safeStringify(r);
