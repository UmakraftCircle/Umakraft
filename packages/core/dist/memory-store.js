...
export class MemoryStore {
    ...
    async saveObservation(obs) {
        try {
            ...
            db.prepare(`INSERT INTO learning_observations (task_id, task_name, tool_slug, error_message, timestamp, context)
         VALUES (?, ?, ?, ?, ?, ?)`).run(obs.taskId, obs.taskName, obs.toolSlug, obs.errorMessage, obs.timestamp, obs.context || null);
    ...
    async loadObservations() {
        try {
            const db = await getDatabase();
            const rows = db.prepare(`SELECT task_id, task_name, tool_slug, error_message, timestamp, context
         FROM learning_observations ORDER BY id DESC LIMIT 1000`).all();
            return rows.map((r) => ({
                taskId: r.task_id,
     ...
     */
    async getToolFailureStats() {
        try {
            const db = await getDatabase();
            const rows = db.prepare(`SELECT tool_slug, COUNT(*) as count, MAX(timestamp) as last_seen
    ...
    async saveRule(rule) {
        try {
            const db = await getDatabase();
            db.prepare(`INSERT INTO adaptation_rules (id, pattern, suggestion, occurrences, last_seen)
         VALUES (?, ?, ?, 1, ?)
         ON CONFLICT(id) DO UPDATE SET
           occurrences = adaptation_rules.occurrences + 1,
           last_seen = excluded.last_seen`).run(rule.id, rule.pattern, rule.suggestion, rule.lastSeen);
        }
        catch (err) {
    ...
    async loadRules() {
        try {
            const db = await getDatabase();
            const rows = db.prepare(`SELECT id, pattern, suggestion, occurrences, last_seen
         FROM adaptation_rules ORDER BY occurrences DESC`).all();
            return rows.map((r) => ({
                id: r.id,
                pattern: r.pattern,
