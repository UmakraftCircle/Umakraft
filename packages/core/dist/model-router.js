...
export class ModelRouter {
    ...
    route(context) {
        const candidates = Array.from(this.profiles.values())
            .filter(m => {
            if (context.requiresStructuredOutput && !m.capabilities.includes('structured-output'))
                return false;
            if (context.requiresVision && !m.capabilities.includes('vision'))
                return false;
            if (context.preferLocal && !m.isLocal)
                return false;
            return true;
        });
