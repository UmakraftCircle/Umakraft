...
export const discordSendMessage = {
...
export const databaseStoreResult = {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            if (tasksList.some((t) => t.status === 'failed')) {
                planStatus = 'failed';
            }
            else if (tasksList.some((t) => t.status === 'running' || t.status === 'pending')) {
                planStatus = 'running';
            }
      ...
      `).run(planId, planData.intent || 'Unknown intent', planData.metadata?.modelUsed || 'unknown-model', planData.metadata?.createdAt || new Date().toISOString(), tasksList.length, planStatus);
      ...
      `);
            for (const task of tasksList) {
                taskStmt.run(task.id, planId, task.name, task.toolSlug, JSON.stringify(task.arguments || {}), JSON.stringify(task.dependencies || []), task.status, task.result ? JSON.stringify(task.result) : null, task.error || null, task.retryCount || 0, task.maxRetries || 3);
            }
