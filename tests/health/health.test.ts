...
describe('Health Domain', () => {
  ...
  it('collects CI and deployment metadata without network access', () => {
    ...
    assert.equal(collectDeploymentHealth({ PANDASTACK_ENVIRONMENT: 'production', PANDASTACK_DEPLOYMENT_ID: 'panda-1' })[0].service, 'deployment');
    assert.equal(collectDeploymentHealth({ PANDASTACK_ENVIRONMENT: 'production' })[0].metadata?.provider, 'PandaStack');
  });
