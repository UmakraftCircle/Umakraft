...
> **Platform note:** This is a role/persona skill, not tied to any vendor. The names below (Pinecone, Weaviate, Qdrant, SharePoint, Confluence, Notion) are *illustrative* technologies, not requirements. Where this skill says "vector DB" or "knowledge graph", in Zaro the practical equivalents are workspace memory (`/.nexus/memory/`), `.table` files (structured/queryable), workspace file search (full-text / phrase / hybrid / semantic), and multi-agent task topics.
